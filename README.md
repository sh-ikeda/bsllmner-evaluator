...
Each category is asked as an independent yes/no question ("does this category's description apply?"), so every category gets its own judgment — there is no single chosen category per stage. Extraction categories are asked whenever `mapping_decision` is `false` and `term_id` is not empty; all of their columns are empty otherwise (the categories were never asked). Selection categories are asked under those same conditions, with one more gate: for JSON input, a value that bsllmner-mk2 resolved via an exact ontology-search/text2term match (Stage 2) never reached its LLM selection step (Stage 3), so the evaluator skips asking selection categories for it — this is detected from the `select_timings` field in the select-output JSON (absent there means Stage 3 was never invoked for that value). TSV input carries no such pipeline detail, so selection categories are always asked for it under the same conditions as extraction. `probability`/`normalized_probability` follow the same definition as columns 6/7, computed for that category's `true`/`false` decision token; `reason` is one short sentence from the model.

Two selection categories, `selection_failed_to_reject` and `selection_better_candidate_available`, are additionally grounded in the actual candidate list bsllmner-mk2's Stage 3 selection step had to choose from for that value: the evaluator reconstructs it from the select-output JSON's `search_results` and `text2term_results` fields (merged and de-duplicated by term_id, mirroring bsllmner-mk2's own candidate merge) and includes it in the prompt so the judgment is about "was the right candidate in this list" rather than the model's own guess at what candidates might have existed. When that candidate list can't be reconstructed — TSV input (which carries no such pipeline detail) or a JSON value with no recorded search/text2term results — these two categories are skipped and their columns stay empty, while the other selection category (`selection_valid`) is still asked as before.
