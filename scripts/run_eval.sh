...
for attr in "$@"; do
    ...
    awk -F "\t" -vOFS="\t" '$3&&!a[$2 $3 $4]++{print $1,gensub(":","_","g",$3)}' ${PREFIX}_result_${attr}.tsv > ${PREFIX}_result_${attr}_uniq_pairs.tsv
    wc -l ${PREFIX}_result_${attr}_uniq_pairs.tsv
    cut -f 1 ${PREFIX}_result_${attr}_uniq_pairs.tsv | while read id; do wget -q -nc -P $BS_JSON_DIR/$attr https://ddbj.nig.ac.jp/search/entry/biosample/$id.json ; done
